    ...
    return (
        <div className="max-w-7xl mx-auto flex flex-col gap-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-[#6B7280] mt-0.5">
                        Kelola data guru dan staff yang terdaftar dalam sistem.
                    </p>
                </div>
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white border border-[#E5E7EB] rounded-xl p-4 flex items-center justify-between shadow-sm">
                    <div className="flex flex-col">
                        <span className="text-xs font-semibold text-[#6B7280] uppercase tracking-wide">Total Guru / Staff</span>
                        <span className="text-2xl font-semibold text-[#1F2937] mt-1">{teachers.length}</span>
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-14 px-4 text-center">
                        ...
                        <h3 className="text-base font-semibold text-[#1F2937]">
                            {teachers.length === 0 ? 'Belum ada data guru / staff.' : 'Tidak ada guru/staff yang cocok.'}
                        </h3>
                        <p className="text-sm text-[#6B7280] max-w-sm mt-1 mb-4">
                            {teachers.length === 0
                                ? 'Mulai dengan menambahkan data tenaga pendidik dan staff pertama Anda.'
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-[#E5E7EB] text-sm">
                                {pagedTeachers.map((teacher) => (
                                    <tr key={teacher.id} className="hover:bg-[#F5F7FA]/60 transition-colors">
                                        <td className="py-2.5 px-4 font-medium text-[#1F2937]">{teacher.nip}</td>
                                        <td className="py-2.5 px-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="font-medium text-[#1F2937]">{teacher.name}</span>
                                            </div>
                                        </td>
                    ...
                    <div className="px-4 py-3 border-t border-[#E5E7EB] flex flex-col sm:flex-row items-center justify-between gap-3 text-sm text-[#6B7280]">
                        <span>
                            ...
                            <strong className="text-[#1F2937] font-semibold">
                                {(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, filteredTeachers.length)}
                            </strong>{' '}
                            dari <strong className="text-[#1F2937] font-semibold">{filteredTeachers.length}</strong> guru/staff
                        </span>
            ...
            <Modal
                ...
                title={editingId ? 'Edit Guru / Staff' : 'Tambah Guru / Staff'}
            >
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        ...
                        {!editingId && (
                            <p className="text-xs text-[#9CA3AF] mt-1">NIP juga akan menjadi password default guru.</p>
                        )}
                    </div>
                ...
                <div className="fixed inset-0 z-50 bg-[#0F172A]/40 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl border border-[#E5E7EB] shadow-2xl max-w-md w-full p-5">
                        <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-3 mb-4">
                            <h3 className="text-base font-semibold text-[#1F2937]">Detail Guru / Staff</h3>
                            <button
                        ...
                        <div className="mt-4 pt-3 border-t border-[#E5E7EB]">
                            <span className="text-[11px] uppercase tracking-wide text-[#9CA3AF] block">Email</span>
                            <span className="text-sm font-medium text-[#1F2937]">{detailTarget.email}</span>
                        </div>
                ...
                <div className="fixed inset-0 z-50 bg-[#0F172A]/40 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl border border-[#E5E7EB] shadow-2xl max-w-sm w-full p-5">
                        <div className="flex items-start gap-3.5">
                            ...
                            <div>
                                <h3 className="text-base font-semibold text-[#1F2937]">Hapus Data Guru / Staff?</h3>
                                <p className="text-sm text-[#6B7280] mt-1.5">
                                    Apakah Anda yakin ingin menghapus{' '}
                                    <strong className="text-[#1F2937]">{deleteTarget.name}</strong>? Tindakan ini
                                    tidak dapat dibatalkan dan akan mencabut seluruh akses akun sistem.
                                </p>
                            </div>
                        </div>

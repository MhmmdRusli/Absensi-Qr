                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            onClick={async () => {
                                try {
                                    const response = await api.get('/admin/subjects/export', {
                                        responseType: 'blob',
                                    });
                                    const url = window.URL.createObjectURL(new Blob([response.data]));
                                    const link = document.createElement('a');
                                    link.href = url;
                                    link.setAttribute('download', 'data-mata-pelajaran.csv');
                                    document.body.appendChild(link);
                                    link.click();
                                    link.remove();
                                    window.URL.revokeObjectURL(url);
                                    setNotice('Data mata pelajaran berhasil diekspor.');
                                } catch (err) {
                                    setNotice('Gagal mengekspor data mata pelajaran.');
                                }
                            }}
